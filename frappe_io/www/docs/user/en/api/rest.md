...
You can create the api keys in two ways:

#### From the User DocType
...
#### From any other DocType
If you are generating keys for a specific DocType's operations (for example: The keys needed for authentication in case of Event Streaming are generated in the Event Consumer) you will have to create an API user and have a user field in the DocType.

In this case, the token is generated as it is specified in the previous method but along with that you need to pass a `Frappe-Authorization-Source` header specifying the DocType which is the source of the keys. For example, in Event Streaming, the Frappe-Authorization-Source is "Event Consumer" as it is the source for the keys.
